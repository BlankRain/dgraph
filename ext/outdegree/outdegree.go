...
func init() {
	ext.RegistProcessFunction("outdegree", OutDegree, 160)
}

func OutDegree(param ext.ProcessFuncParam) (map[uint64]types.Val, error) {
	...
	for _, edgePred := range param.ParamLabels {
		...
		if err != nil {
			log.Printf("error when compute outdegree %v", err)
			return nil, err
		}
	}
	// trans to types.Val
